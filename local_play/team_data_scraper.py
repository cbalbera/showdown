...
def getPlayerIDs(teams):
    ...
    for i in teams_to_add:
        #print(f"adding {i} to teams_to_pull")
        teams_to_pull.append(teams_list[i.strip()])
    #print(f"teams to pull is {teams_to_pull}")
    for i in teams_to_pull:
        ...
        #print("hitters and pitchers looked up")
        ...
        for j in hitters:
            ...
            #print(f"looking up hitter id {j}")
        ...
        for j in pitchers:
            #print(f"looking up pitcher id {j}")
            players = statsapi.lookup_player(j.rstrip())
            for player in players:
                if player["currentTeam"]["id"] == i and player["id"] not in player_IDs:
                    player_IDs.append(player["id"])
